...
Có 2 trường hợp mình nghĩ thường tới **full text search**, --đó là khi mình-- cần query với từ khóa mềm dẻo hơn một tí, và khi mình cần cần tốc độ với những bảng chứa nhiều dữ liệu.

Khi mà bạn dùng <mark>LIKE</mark> (hoặc <mark>ILIKE</mark>) thì bắt buộc keyword của người dùng phải đúng thứ tự, không được thiếu chữ ở giữa. Nếu tài liệu bạn cần tìm là "Báo cáo cho tháng 10", thì bạn không thể tìm ra với những từ khóa như "báo cáo tháng 10", "tháng 10 báo cáo", "báo cáo 10" ... được.
...
#### 2. Cơ chế
Về cơ bản thì cách làm việc của full text search sẽ dựa trên <mark>ts_vector</mark>, <mark>ts_query</mark> và <mark>@@</mark>, là một vài hàm có sẵn trong PostgreSQL, các hệ quản trị CSQL khác hẳn cũng có các hàm này hoặc tương tự.

<mark>to_tsvector</mark> sẽ chuyển dữ liệu kiểu string thành kiểu vector.

```sql
-- thử xem bên trong thế nào
...
```
(such as **Cabernet Sauvignon, Chardonnay, Pinot noir, Sauvignon blanc, Syrah**)
```ruby
# /config/routes.rb
...
<%= @hello %>
<%= @@test %>
<% Mã Ruby -- nội tuyến với đầu ra %>
<%= Biểu thức Ruby -- thay thế bằng kết quả %>
<%# bình luận -- bị bỏ qua -- hữu ích trong việc thử nghiệm %>
% một dòng mã Ruby -- được coi là <% dòng %> (tùy chọn -- xem ERB.new)
%% được thay thế bằng % nếu điều đầu tiên trên một dòng và % xử lý được sử dụng
<%% hoặc %%> -- thay thế bằng <% hoặc %> tương ứng
```
```ruby
# check thử xêm tiếng Ziệt nó thế lào o O 0 `tr` 'ty' "th" { shit @}
# ==, ===, !=, <=>, >, <, >=, <=, <=>, ===, .eql?, equal?, hash
